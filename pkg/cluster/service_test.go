...
func clustersReady(req *cluster.RequestWithClusters) (bool, error) {
	for _, c := range req.Clusters {
		...
		ok := c.Status == "normal" &&
			...
			c.WorkshopURL == fmt.Sprintf("https://redhat-scholars.github.io/openshift-starter-guides/rhs-openshift-starter-guides/4.5/index.html?CLUSTER_SUBDOMAIN=%s&USERNAME=%s&PASSWORD=%s&LOGIN=%s", c.Hostname, c.User.ID, c.User.Password, encodedLoginURL) &&
